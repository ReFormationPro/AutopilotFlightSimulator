...
from cmds import AltitudeCommand, AltitudePeriod, Command, DistanceCommand, EndCommand, FakeCommand, LedCommand, LedValue, ManualCommand, PressCommand, SpeedCommand
...
class PeriodicAgent(Agent):
    ...
    def on_period_finished(self, timestamp: float, period_number: int):
        if self.period_status == PeriodStatus.SUCCESS:
            ...
            logger.info(
                f"{type(self).__name__} has ignored the period number {period_number} at {timestamp}")
...
class DistanceAgent(PeriodicAgent):
    ...
    def send_speed_cmd(self):
        ...
        speed_cmd = SpeedCommand(speed)    # TODO Add other distance calculation strategies
        logger.info(f"Sending speed cmd for speed {speed} at {self.relative_time()}")
    ...
    def attempt_cmd(self, timestamp: float, period_number: int, cmd: Command) -> PeriodStatus:
        ...
        if type(cmd) == AltitudeCommand:
            ...
            if self.next_expected_altitude == None:
                self.period_status = PeriodStatus.FAILURE
                logger.error(f"AltitudeControllerAgent no {self.controller_idx} received an unexpected altitude message for period {period_number} at {timestamp}")
...
class FakeCommandAgent(Agent):
    def __init__(self, time: float, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.time = time
        AlarmAgent.instance().add_alarm(self.on_alarm, self.to_real_time(self.time))

    def on_alarm(self):
        logger.info(
            f"FakeCommandAgent is sending fake command at {self.relative_time()}")
        self.send_command(FakeCommand(b"ZZZ"))
