...
class FakeCommand(Command):
    """
    Fake command for FakeCommandAgent
    Does not have a valid ID
    """
    MSG_ID = None

    def __init__(self, content_buffer: bytes):
        self.content_buffer = content_buffer

    def make_bytes(self):
        return CMD_START_BYTE + self.content_buffer + CMD_END_BYTE
    
    @classmethod
    def _parse_bytes(cls, buffer: bytes):
        if CMD_START_BYTE in buffer or CMD_END_BYTE in buffer:
            logging.warning(f"FakeCommand content buffer has start or end byte in it")
        return FakeCommand(buffer)
