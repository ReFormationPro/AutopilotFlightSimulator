...
CHAR_DELAY = SETTINGS["CHAR_DELAY"]
...
class AutoPilot:
    ...
    def write(self, message: bytes | Command):
        ...
        if issubclass(type(message), Command):
            self._write(message.make_bytes())
    ...
    def _write(self, message: bytes):
        """
        On different computers we may need to wait a bit
        ...
        if CHAR_DELAY == 0:
            ...
            with self.writer_lock:
                for b in message:
                    ...
                    self.serial.write(b.to_bytes(1, byteorder="big"))
    ...
    def agents_demo(self):
        ...
        fake_command_agents = []
        if TESTCASE.get("fake-command-times"):
            for fake_command_time in TESTCASE["fake-command-times"]:
                fake_command_agents.append(FakeCommandAgent(fake_command_time, TESTCASE, self))
